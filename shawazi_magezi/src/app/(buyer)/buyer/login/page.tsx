    ...
    try {
      ...
      if (loginResponse) {
        if (rememberMe) {
          ...
          // Store login data securely (consider encryption in a real-world scenario)
    ...
    <div className="min-h-screen bg-white flex flex-col justify-center relative overflow-hidden">
      ...
      <div className="w-full max-w-xs sm:max-w-sm md:max-w-lg lg:max-w-xl xl:max-w-2xl mx-auto z-10 bg-white p-6 sm:p-8 rounded-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {error && (
            <div className="text-border-color text-center mt-2">{error}</div>
          )}
        </form>
...
//                 router.push(`/buyer/otp-verification?phone_number=${encodeURIComponent(data.phone_number)}`);
//             } else {
//                 setError('Login failed. Please check your credentials.');
//             }
//         } catch (error) {
