...
// Define the interface for the login form data that matches exactly what we're sending
interface LoginFormData {
  phone_number: string;
  password: string;
  role?: string; // Make role optional in the form
}
...
// Update the schema to match the LoginFormData interface exactly
const schema = yup.object().shape({
  phone_number: yup.string().required("Phone number is required"),
  password: yup.string().required("Password is required"),
  role: yup.string().optional() // Make role optional in validation
});
...
const Login = () => {
  const { 
    ...
    resolver: yupResolver<LoginFormData>(schema), // Add type parameter here
    mode: 'onChange',
    defaultValues: {
      role: 'user' // Set a default value for role
    }
  });
  ...
  const onSubmit = async (data: LoginFormData) => {
    ...
    try {
      // Add the role to the data before sending
      const loginData = {
        ...data,
